...
def tweet_select_by_keywords(df_before_match, kind_list, param1=True):
    '''
    通过关键词匹配用户帖子
    :param df_before_match: 清洗后的非水军帖子
    :param kind_list: 关键词对应的列表
    :param param1: True为匹配，False为筛除
    :return:
    '''
    df_relate = df_before_match[df_before_match.content_clean.str.contains(kind_list[0])]
    kind_length = len(kind_list)
    for i in range(1, kind_length):
        if param1:
            df_layout = df_before_match[df_before_match.content_clean.str.contains(kind_list[i])]
        else:
            df_layout = df_before_match[-df_before_match.content_clean.str.contains(kind_list[i])]
